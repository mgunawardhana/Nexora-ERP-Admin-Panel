...
interface ProductOrder {
	...
	date: string;
	time: string;
	exotic: boolean; // FIX: Changed from number to boolean
	demonstratorName: string; // ADD: New field from API
}
...
function BookingType() {
					...
					const flattenedProductOrders = ordersData.flatMap((order) =>
						order.orders.flatMap((subOrder) =>
							subOrder.selectedProducts.map((product) => ({
								...
								date: order.date,
								time: order.time,
								exotic: subOrder.exotic || false, // FIX: Map boolean value
								demonstratorName: subOrder.demonstratorName || 'N/A' // ADD: Map new field
							}))
						)
					);
			...
			try {
				setProductOrders(
					productOrders.filter(
						(item) =>
							!(item.orderCode === itemToDelete.orderCode && item.productId === itemToDelete.productId)
					)
				);
	...
	const tableColumns = [
		...
		{ title: t('Quantity'), field: 'quantity', cellStyle: { paddingTop: 16, paddingBottom: 16 } },
		{
			title: t('Exotic'),
			field: 'exotic',
			render: (rowData: ProductOrder) => (rowData.exotic ? t('Yes') : t('No')), // FIX: Render boolean as text
